...
bool check(int32 i, int32 j, int32 t1, int32 t2) {
    ...
    if (i + sd[t1] - 1 < 1 || i + sd[t1] - 1 > m) return false;
    if (j + sd[t2] - 1 < 1 || j + sd[t2] - 1 > n) return false;
    if (visited[i][j][t1][t2]) return false;
...
void BFS() {
    ...
    while(!q.empty()) {
        TYPE top = q.front(); q.pop();
        //Tinh tien
        rep(i, 0, 3) {
            int32 u = top.i + movex[i], v = top.j + movey[i];
            if (!check(u, v, top.t1, top.t2)) continue;
